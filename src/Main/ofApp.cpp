	...
	if (currentState != nullptr){
		...
		if(currentState->hasFinished()){
			if(currentState->getNextState() == "Menu"){
				...
				//Turn off the other sounds 
				winState_sound.stop();
				loseState_sound.stop();
				//Sets the game sound on a loop
				...
				sound.stop();
				loseState_sound.load("youLose.mp3");
				loseState_sound.play();
			} else if (currentState->getNextState() == "Win" ) {
				currentState = winState;
				sound.stop();
				winState_sound.load("youWin.mp3");
				winState_sound.play();
			}
			currentState->reset();
		}
